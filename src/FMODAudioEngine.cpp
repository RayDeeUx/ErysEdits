...
class $modify(MyFMODAudioEngine, FMODAudioEngine) {
	...
	void playEffect(const gd::string& p0, const float p1, const float p2, const float p3) {
		if (std::find(m_fields->manager->badSFX.begin(), m_fields->manager->badSFX.end(), std::string(p0)) == m_fields->manager->badSFX.end() || Utils::isSceneRunning("PauseLayer") || !(Utils::modEnabled() && Utils::get("disableRewardsSFX")) || !m_fields->manager->isPlayerDead) {
